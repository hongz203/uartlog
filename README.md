...
4. `Theme: Dark/Light` 버튼으로 테마 전환
5. `Add Filter Tab`으로 탭 추가
6. 각 탭에서 `Keyword / Regex`, `Match case` 설정
7. 각 탭에서 `FG`, `BG` 색상을 골라 키워드 하이라이트 확인
8. `Filter Panel: Right/Bottom` 버튼으로 필터 패널 위치 변경 후 splitter로 크기 조절
9. 필요 시 `Paused` 또는 `Clear All` 사용
